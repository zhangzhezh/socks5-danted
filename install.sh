...
Getserverip_n=$(ifconfig | grep 'inet addr' | grep -Ev 'inet addr:127.0.0' | sed -n 's/.*inet addr:\([^ ]*\) .*/\1/p' | wc -l)
Getserverip=$(ifconfig | grep 'inet addr' | grep -Ev 'inet addr:127.0.0' | sed -n 's/.*inet addr:\([^ ]*\) .*/\1/p')
